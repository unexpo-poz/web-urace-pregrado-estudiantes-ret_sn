...
function resaltar(lSeccion) {
	...
	// Si retira del 1ro al 3ro
	materia = lSeccion.value.split(" ");
	if (materia[5] == '-1') {//Si la esta retirando
     
          msg = 'Solo se permite el retiro de una asignatura.\n';
          alert (msg);
		
		/*masAlto = semestreAlto(document.pensum);

		if ((masAlto >= 5) && (materia[9] < 5)) {
			msg = 'Disculpe, esta intentando retirar una asignatura necesaria para poder cursar semestres mas avanzados.\n\n';
			msg+= 'Para mas informacion consulte la Normativa para Retiro de Asignaturas en nuestro Marco Legal.';
			noRetira = true;
		}*/
	}

	// DESHABILITADO TEMPORALMENTE EL 16/11/2012 POR CDR-2012-32-06
	// Si retira mas del 70%
	/*if (document.totales.t_uc.value < porc) {
		msg = 'Disculpe, esta intentando retirar mas asignaturas de los permitido.\n';
		msg+= 'Para mas informacion consulte la Normativa para Retiro de Asignaturas en nuestro Marco Legal.';
		noRetira = true;
	}*/
...
function FechaValida(dia,mes,anyo)
 {
  ...
  if(anyoBisiesto(anyo))
   ...
   if ((anyo<1935) || (yearhoy - anyo < 15))
    {
     Mensaje += "- Año de nacimiento___ inválido\r\n" + anyo;
    } 
   if (Mensaje != "")
   {
	   //alert(Mensaje);
...
function porcentajeInscritas() {
	var pordentaje = "";
	porcentaje = Math.round((document.pensum.uc_insc.value * 70) / 100);
	return (porcentaje);
}

function semestreAlto(fp) {
    ...
    with (fp) {
        for(j=0;j < (CB.length - 1); j++){
            ...
            if (cod_uc[5] != '-1'){ // Si no esta retirada
                                   
                temp = parseInt( "0"+cod_uc[9],10);// Capturo semestre de la materia
